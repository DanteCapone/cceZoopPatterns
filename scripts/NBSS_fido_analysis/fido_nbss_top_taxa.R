...
librarian::shelf(
 ...
 base_dir <- here::here("scripts", "NBSS_fido_analysis")
...
if (!is.null(results_coi)) {
  write.csv(
    ...
    file.path(outputs_dir, "fido_nbss_coi_posterior_summary.csv"),
    row.names = FALSE
  )
...
if (!is.null(results_18s)) {
  write.csv(
    ...
    file.path(outputs_dir, "fido_nbss_18s_posterior_summary.csv"),
    row.names = FALSE
  )
