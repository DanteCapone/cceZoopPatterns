...
##First Size 1
#Phyloseq Filtered
fido_input_filt=read.csv(here("data/fido/phy/fido_coi_s1_ecdf_order_phy.csv"), header=TRUE, check.names = FALSE, row.names = 1) %>%
...
write.csv(final_data_s1,here(paste0("data/predicted_og/predicted_og_coi_",current_date,"_s1_phy_order.csv")))
...
fido_input_filt=read.csv(file.path("data/fido/phy/fido_coi_s2_ecdf_order_phy.csv"), header=TRUE, check.names = FALSE, row.names = 1)%>%
  column_to_rownames("Order")
#Metadata
...
current_date <- format(Sys.Date(), "%m_%d_%Y")
write.csv(final_data_s2,here(paste0("data/predicted_og/predicted_og_coi_",current_date,"_s2_phy_order.csv")))


# S3 ----------------------------------------------------------------------

fido_input_filt=read.csv(file.path("data/fido/phy/fido_coi_s3_ecdf_order_phy.csv"), header=TRUE, check.names = FALSE, row.names = 1)%>%
  column_to_rownames("Order")
...
current_date <- format(Sys.Date(), "%m_%d_%Y")
write.csv(final_data_s3,here(paste0("data/predicted_og/predicted_og_coi_",current_date,"_s3_phy_order.csv")))
...
files <- list.files(file_dir, pattern = "predicted_og_coi_", full.names = TRUE)

# Archive files with an older date
for (file in files) {
  date_str <- str_extract(basename(file), "(?<=predicted_og_coi_)[0-9_]+(?=_s\\d_phy_order\\.csv)")
