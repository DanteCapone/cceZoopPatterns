...
fido_s1=read.csv(here("data/predicted_og/predicted_og_18s_04_03_2024_s1_phy.csv")) %>%
  select(-X)
fido_s2=read.csv(here("data/predicted_og/predicted_og_18s_04_03_2024_s2_phy.csv")) %>%
  select(-X)
fido_s3=read.csv(here("data/predicted_og/predicted_og_18s_04_03_2024_s3_phy.csv")) %>%
  select(-X)
#Merge
...
#Taxa
zhan_taxa_all=read.csv(here("data/phyloseq_bio_data/18S/fido_18s_family_tax_table.csv")) %>% 
  select(-X)

zhan_taxa= zhan_taxa_all %>% 
  filter(Family %in% final_data_all_sizes$coord) %>% 
  distinct() %>% 
