...
fido_s1=read.csv(here("data/predicted_og/predicted_og_18s_04_15_2024_s1_phy_order.csv")) %>%
  select(-X)
fido_s2=read.csv(here("data/predicted_og/predicted_og_18s_04_15_2024_s2_phy_order.csv")) %>%
  select(-X)
fido_s3=read.csv(here("data/predicted_og/predicted_og_18s_04_15_2024_s3_phy_order.csv")) %>%
  select(-X)
#Merge
...
#Taxa
zhan_taxa_all=read.csv(here("data/phyloseq_bio_data/18S/fido_18s_order_tax_table.csv")) %>% 
  select(-X)

zhan_taxa= zhan_taxa_all %>% 
  filter(Order %in% final_data_all_sizes$coord) %>% 
  distinct() %>% 
...
#Predicted proportions
fido_s1_raw=read.csv(here("data/fido/phy/fido_18s_s1_ecdf_order_phy.csv")) %>% 
  select(-starts_with("X")) %>% 
  pivot_longer(cols = -Order, names_to = "Sample_ID", values_to = "n_reads") %>%
  mutate(Sample_ID_short= str_extract(Sample_ID, ".*(?=\\.[^.]+$)")) %>%
  group_by(Sample_ID_short, Order) %>%
...
fido_s2_raw=read.csv(here("data/fido/phy/fido_18s_s2_ecdf_order_phy.csv")) %>% 
  select(-starts_with("X")) %>% 
  pivot_longer(cols = -Order, names_to = "Sample_ID", values_to = "n_reads") %>%
  mutate(Sample_ID_short= str_extract(Sample_ID, ".*(?=\\.[^.]+$)")) %>%
  group_by(Sample_ID_short, Order) %>%
...
fido_s3_raw=read.csv(here("data/fido/phy/fido_18s_s3_ecdf_order_phy.csv")) %>% 
  select(-starts_with("X")) %>% 
  pivot_longer(cols = -Order, names_to = "Sample_ID", values_to = "n_reads") %>%
  mutate(Sample_ID_short= str_extract(Sample_ID, ".*(?=\\.[^.]+$)")) %>%
  group_by(Sample_ID_short, Order) %>%
...
merge(fido_s1_raw, fido_s2_raw, by = "Order", all = TRUE) %>%
  merge(.,fido_s3_raw, by = "Order", all = TRUE)%>%
  column_to_rownames("Order") %>%
