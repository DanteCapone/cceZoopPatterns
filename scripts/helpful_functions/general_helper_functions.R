...
# Function to perform CLR transformation on a vector
clr_convert <- function(proportions_vector) {
  
  # Compute log values
  log_values <- log(proportions_vector)
  ...
  if (length(valid_log_values) == 0) {
    # If all values are invalid, return a vector of NaN to indicate an issue
    ...
    clr_values <- log(proportions_vector / geometric_mean)
    
    # Replace any remaining Inf or NaN with a specific value (e.g., NaN or a chosen placeholder)
