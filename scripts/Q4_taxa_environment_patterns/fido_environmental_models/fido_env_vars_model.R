...
posterior_summary_all%>% 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(size = 12),  # Adjust font size for x-axis tick labels
        axis.text.y = element_text(size = 12)) ->all_plot_18s
...
here()
#coi read in different sizes
otu18s1_pcrra=read.csv(here("data/predicted_og/predicted_og_18s_02_22_2024_s1_phy.csv")) %>%
  filter(cycle_num==0) %>% 
...
otu18s2_pcrra=read.csv(here("data/predicted_og/predicted_og_18s_02_22_2024_s2_phy.csv")) %>%
  filter(cycle_num==0) %>% 
  mutate(Sample_ID = gsub('predicted ', '', replicate)) %>% 
  select(n_reads,coord,Sample_ID) %>% 
  pivot_wider(names_from = Sample_ID, values_from = n_reads) %>% 
...
otu18s3_pcrra=read.csv(here("data/predicted_og/predicted_og_18s_02_22_2024_s3_phy.csv")) %>%
  filter(cycle_num==0) %>% 
  mutate(Sample_ID = gsub('predicted ', '', replicate)) %>% 
  select(n_reads,coord,Sample_ID) %>% 
  pivot_wider(names_from = Sample_ID, values_from = n_reads) %>% 
