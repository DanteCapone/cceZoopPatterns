...
fido_s1=read.csv(here("data/predicted_og/predicted_og_18s_04_17_2024_s1_phy_all_and_subpools.csv")) %>%
  ...
  mutate(replicate = gsub("predicted ", "", replicate), Family=coord) %>%
  select(Family,n_reads, replicate) %>%
  # Pivot wider
...
common_families <- intersect(rownames(fido_s1), rownames(otu18s1))
common_samples <- intersect(colnames(fido_s1), colnames(otu18s1))

fido_s1 <- fido_s1[common_families, common_samples]
otu18s1 <- otu18s1[common_families, common_samples]

# Element-wise multiplication of both tables
fido_s1_counts <- fido_s1 * otu18s1
fido_s1_counts=otu_table(fido_s1_counts, taxa_are_rows = TRUE)


# Ensure both matrices have the same row and column names
common_families <- intersect(rownames(fido_s2), rownames(otu18s2))
common_samples <- intersect(colnames(fido_s2), colnames(otu18s2))

fido_s2 <- fido_s2[common_families, common_samples]
otu18s2 <- otu18s2[common_families, common_samples]


fido_s2_counts <- fido_s2 * otu18s2
fido_s2_counts=otu_table(fido_s2_counts, taxa_are_rows = TRUE)


# Ensure both matrices have the same row and column names
common_families <- intersect(rownames(fido_s3), rownames(otu18s3))
common_samples <- intersect(colnames(fido_s3), colnames(otu18s3))

fido_s3 <- fido_s3[common_families, common_samples]
otu18s3 <- otu18s3[common_families, common_samples]


fido_s3_counts <- fido_s3 * otu18s3
...
dat_1=phyloseq(fido_s1_counts,zhan_taxa,meta18s) %>% merge_samples(.,"Sample_ID_short",fun= mean)
dat_2=phyloseq(fido_s2_counts,zhan_taxa,meta18s)%>% merge_samples(.,"Sample_ID_short",fun= mean)
dat_3=phyloseq(fido_s3_counts,zhan_taxa,meta18s)%>% merge_samples(.,"Sample_ID_short",fun= mean)
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q4_taxa_vars/fido_env_vars_18s_taxa_pcr_ra_heatmap_50.pdf"), 
    plot = heatmap_plot_18s_pcrra,
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q4_taxa_vars/fido_env_vars_18s_taxa_pcr_ra_heatmap_50.png"), 
    plot = heatmap_plot_18s_pcrra,
    width = 16,  # Width in inches
    height = 16  # Height in inches
  )}
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q4_taxa_vars/fido_env_vars_18s_taxa_pcr_ra_95.pdf"), 
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q4_taxa_vars/fido_env_vars_18s_taxa_pcr_ra_95.png"), 
    plot = all_plot_18s_pcrra,
    width = 12,  # Width in inches
    height = 16  # Height in inches
  )}
...
#Predicted proportions
#Reformat for phyloseq
otucoi1_pcr=read.csv(here("data/predicted_og/predicted_og_coi_02_26_2024_s1_phy.csv")) %>%
  select(-X) %>%
  # Filter out rows where 'replicate' column contains 'predicted'
  filter(grepl("predicted", replicate)) %>%
  # Remove 'predicted' from the 'replicate' column
...
#S2
otucoi2_pcr=read.csv(here("data/predicted_og/predicted_og_coi_02_26_2024_s2_phy.csv")) %>%
  select(-X) %>%
  # Filter out rows where 'replicate' column contains 'predicted'
  filter(grepl("predicted", replicate)) %>%
  # Remove 'predicted' from the 'replicate' column
...
#S3
otucoi3_pcr=read.csv(here("data/predicted_og/predicted_og_coi_02_26_2024_s3_phy.csv")) %>%
  select(-X) %>%
  # Filter out rows where 'replicate' column contains 'predicted'
  filter(grepl("predicted", replicate)) %>%
  # Remove 'predicted' from the 'replicate' column
...
dat_1=phyloseq(otucoi1_pcr,coi_taxa,metacoi) %>% merge_samples(.,"Sample_ID_short",fun= mean)
dat_2=phyloseq(otucoi2_pcr,coi_taxa,metacoi)%>% merge_samples(.,"Sample_ID_short",fun= mean)
dat_3=phyloseq(otucoi3_pcr,coi_taxa,metacoi)%>% merge_samples(.,"Sample_ID_short",fun= mean)
...
#Plot
all_plot_pcr_coi=ggplot(posterior_summary_all, aes(x = mean, y = coord, color=size)) + 
  geom_vline(aes(xintercept = 0), color = "black", alpha=0.4, size=1)+
  geom_errorbarh(aes(xmin = p2.5, xmax = p97.5), height = 0.2, size=1) +
  geom_point(aes(shape=size),size = 8, alpha=0.8) + 
  labs(title="Pibble Model for COI Taxa vs. PC1 (PCR Bias-Corrected)",x = "Offfshore \u2190 Centered Log-Ratio(PC1) \u2192 Onshore",
       y="", color = "Size Fraction (mm)", shape="Size Fraction (mm)") +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 14),  # Adjust font size for x-axis tick labels
        axis.text.y = element_text(size = 14)) +
  theme(axis.title.x = element_text(size = 14)) +
  theme(legend.text = element_text(size = 14))  # Adjust size here
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q3_taxa_vars/fido_PC1_coi_taxa_pcr.pdf"), 
    plot = all_plot_pcr_coi,
    width = 16,  # Width in inches
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q3_taxa_vars/fido_PC1_coi_taxa_pcr.png"), 
    plot = all_plot_pcr_coi,
    width = 16,  # Width in inches
    height = 6  # Height in inches
  )}
...
all_plot_18s=grid.arrange(pp1,pp2,pp3, nrow=3,
...
all_plot_18s=ggplot(posterior_summary_18s, aes(x = mean, y = coord, color=size)) + 
  ...
  geom_errorbarh(aes(xmin = p2.5, xmax = p97.5), height = 0.2, size=1) +
  geom_point(aes(shape=size),size = 8, alpha=0.8) + 
  labs(title="Pibble Model for 18S Taxa vs. PC1 (PCR Bias-Corrected)",x = "Offfshore \u2190 Centered Log-Ratio(PC1) \u2192 Onshore",
       y="", color = "Size Fraction (mm)", shape="Size Fraction (mm)") +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 14),  # Adjust font size for x-axis tick labels
        axis.text.y = element_text(size = 14)) +
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q3_taxa_vars/fido_PC1_18s_taxa_PCR.pdf"), 
    plot = all_plot_18s,
    width = 16,  # Width in inches
    height = 6  # Height in inches
  )}
...
if (saving==1) {
  ggsave(
    filename = here("plots/Q3_taxa_vars/fido_PC1_18s_taxa_PCR.png"), 
    plot = all_plot_18s,
    width = 16,  # Width in inches
    height = 6  # Height in inches
  )}
