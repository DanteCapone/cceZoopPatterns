...
here()
#18s read in different sizes
otu18s1=read.csv(here("data/fido/fido_18s_s1_ecdf.csv"), row.names = 1)%>%
...
otu18s2=read.csv(here("data/fido/fido_18s_s2_ecdf.csv"), row.names = 1)%>%
  dplyr::select(where(~ !is.na(.[[1]])))%>% 
  select(starts_with("C"))%>%
  rownames_to_column("Hash") %>%
  pivot_longer(-Hash, names_to = "sample", values_to = "value") %>%
...
otu18s3=read.csv(here("data/fido/fido_18s_s3_ecdf.csv"), row.names = 1)%>%
  dplyr::select(where(~ !is.na(.[[1]])))%>% 
  select(starts_with("C"))%>%
  rownames_to_column("Hash") %>%
  pivot_longer(-Hash, names_to = "sample", values_to = "value") %>%
...
tax18s=read.csv(here("data/metazoopruned18s_tax.csv"),row.names = 1)%>% add_row()
rownames(tax18s)[nrow(tax18s)] <- 'other'
#Subset to match 
tax18s1 = tax18s %>% filter(rownames(tax18s) %in% rownames(otu18s1))
tax18s1=  tax_table(as.matrix(tax18s1))
tax18s2 = tax18s %>% filter(rownames(tax18s) %in% rownames(otu18s2))
tax18s2=  tax_table(as.matrix(tax18s2))
tax18s3 = tax18s %>% filter(rownames(tax18s) %in% rownames(otu18s3))
tax18s3=  tax_table(as.matrix(tax18s3))
...
dat_1=phyloseq(otu18s1,tax18s1,meta18s) %>% merge_samples(.,"Sample_ID_short",fun= mean)
dat_2=phyloseq(otu18s2,tax18s2,meta18s)%>% merge_samples(.,"Sample_ID_short",fun= mean)
dat_3=phyloseq(otu18s3,tax18s3,meta18s)%>% merge_samples(.,"Sample_ID_short",fun= mean)
...
for (dat_name in names(data_list)) {
  ...
  tax <- tax_table(dat)[,c("Order","Genus","Species")]
  hash <- rownames(tax_table(dat))[1]
  ...
  filename <- paste0(save_path,"S_", dat_name, "_multiple_vars_18s.png")
