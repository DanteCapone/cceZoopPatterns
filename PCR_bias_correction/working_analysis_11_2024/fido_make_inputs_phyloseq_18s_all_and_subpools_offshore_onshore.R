...
## Separate out by onshore/offshore cluster
...
# Modify the clusters dataframe to match the format of all_runs column names
clusters <- clusters %>%
  mutate(
    Matching_ID = Sample_ID %>%              # Create a new column for matching
      str_replace_all("-", ".")              # Replace all '-' with '.'
  )

# Use the `offshore_onshore` column to separate samples
onshore_samples <- clusters %>%
  filter(offshore_onshore == "onshore") %>%
  pull(Matching_ID)

offshore_samples <- clusters %>%
  filter(offshore_onshore == "offshore") %>%
  pull(Matching_ID)

# Identify columns in all_runs that contain the Matching_ID substring for onshore and offshore
onshore_columns <- colnames(all_runs)[
  sapply(colnames(all_runs), function(col) any(str_detect(col, paste(onshore_samples, collapse = "|"))))
]

offshore_columns <- colnames(all_runs)[
  sapply(colnames(all_runs), function(col) any(str_detect(col, paste(offshore_samples, collapse = "|"))))
]

# Identify additional columns containing "All" or "B3" (case-insensitive match)
additional_columns <- colnames(all_runs)[
  str_detect(colnames(all_runs), regex("All|B3", ignore_case = TRUE))
]

# Combine columns for onshore and offshore, ensuring uniqueness
onshore_columns <- unique(c(onshore_columns, additional_columns))
offshore_columns <- unique(c(offshore_columns, additional_columns))

# Subset the all_runs dataframe without modifying its column names
onshore <- all_runs %>%
  select(all_of(onshore_columns))

offshore <- all_runs %>%
  select(all_of(offshore_columns))
...
unidentified=offshore %>% 
  filter(rownames(offshore) %in% setdiff(rownames(offshore), rownames(taxa_18s))) %>% 
...
#S1
fido_18s_s1=offshore%>%
  dplyr::select(c(contains("All"),
                  contains("B3"), contains("S1"))) %>% 
  filter(rowSums(.) != 0) 

fido_18s_s2=offshore %>%
  dplyr::select(c(contains("All"),
                  contains("B3"), contains("S2"))) %>% 
  filter(rowSums(.) != 0) 

fido_18s_s3=offshore %>%
  dplyr::select(c(contains("All"),
                  contains("B3"), contains("S3"))) %>% 
  filter(rowSums(.) != 0) 
...
write.csv(tax18s_family,here("PCR_bias_correction/data/phyloseq_bio_data/18S/fido_18s_family_tax_table_offshore.csv"))
...
experiments <- c("AllPool","Pooled.B3.B5")
...
#Save
write.csv(fido_18s_s1_save_family_phy,here("PCR_bias_correction/data/fido/phy/fido_18s_s1_ecdf_family_phy_all_subpools_offshore.csv"))
...
#Save
write.csv(fido_18s_s2_save_family_phy,here("PCR_bias_correction/data/fido/phy/fido_18s_s2_ecdf_family_phy_all_subpools_offshore.csv"))
...
#Save
write.csv(fido_18s_s3_save_family_phy,here("PCR_bias_correction/data/fido/phy/fido_18s_s3_ecdf_family_phy_all_subpools_offshore.csv"))
...
unidentified=onshore %>% 
  filter(rownames(onshore) %in% setdiff(rownames(onshore), rownames(taxa_18s))) %>% 
  mutate(total=rowSums(.))
...
fido_18s_s1=onshore%>%
  dplyr::select(c(contains("All"),
                  contains("C5"), contains("S1"))) %>% 
  filter(rowSums(.) != 0) 

fido_18s_s2=onshore %>%
  dplyr::select(c(contains("All"),
                  contains("C5"), contains("S2"))) %>% 
  filter(rowSums(.) != 0) 

fido_18s_s3=onshore %>%
  dplyr::select(c(contains("All"),
                  contains("C5"), contains("S3"))) %>% 
  filter(rowSums(.) != 0) 
...
write.csv(tax18s_family,here("PCR_bias_correction/data/phyloseq_bio_data/18S/fido_18s_family_tax_table_onshore.csv"))
...
#sample in the final df
# Separate rows based appearance in the calibration samples
experiments <- c("AllPool","Pooled.C5.B5")
treatments <- c("20C", "24C", "28C")
replicates <- c("1", "2", "3")
...
#Save
write.csv(fido_18s_s1_save_family_phy,here("PCR_bias_correction/data/fido/phy/fido_18s_s1_ecdf_family_phy_all_subpools_onshore.csv"))
...
#Save
write.csv(fido_18s_s2_save_family_phy,here("PCR_bias_correction/data/fido/phy/fido_18s_s2_ecdf_family_phy_all_subpools_onshore.csv"))
...
#Save
write.csv(fido_18s_s3_save_family_phy,here("PCR_bias_correction/data/fido/phy/fido_18s_s3_ecdf_family_phy_all_subpools_onshore.csv"))
